...
const BookCourse = () => {
     ...
     * Backend creates CourseBooking (PENDING) and returns Stripe clientSecret
     */
    const handlePayNow = async () => {
        ...
        try {
            ...
            if (response.status === 'success' && response.data) {
                // Step 4: Backend returns clientSecret
                const { bookingId, amount, currency, clientSecret: secret, paymentIntentId } = response.data;
                ...
                // Move to payment step — Stripe Elements will render
    ...
    /**
     * Step 5 completed: Stripe payment confirmed
     * Step 6: Webhook on backend handles the rest automatically
     * Step 7: Show success
     */
    const handlePaymentSuccess = (paymentIntent) => {
        ...
        // Redirect to booking confirmation page (Step 7)
        setTimeout(() => {
            setShowSuccessModal(false);
            if (bookingData?.bookingId) {
                navigate(`/personal/bookings/${bookingData.bookingId}`);
            } else {
                navigate('/personal/my-bookings');
            }
        }, 2500);
    };
    ...
    const handlePaymentError = (errorMessage) => {
        ...
        // User stays on payment form, error is shown inside StripePayment
    };
    ...
    return (
        <div className="w-full min-h-screen flex justify-center py-10 px-4 sm:px-8 bg-white lg:bg-gray-50 overflow-y-auto">
            <div className="w-full max-w-xl bg-white lg:rounded-2xl lg:shadow-md p-2 sm:p-8 h-auto flex flex-col mb-10">
                ...
                {paymentStep === 'payment' && clientSecret && (
                    <>
                        ...
                        <StripePayment
                            ...
                            currency={bookingData?.currency || course.currency}
                ...
                {/* ========== BOOKING STEP: Session & Document Selection ========== */}
                {paymentStep === 'booking' && (
                    <>
                        ...
                        {/* Pay Now Button - triggers checkout API */}
                        <button
            ...
            {showSuccessModal && (
                <div className="fixed inset-0 bg-black bg-opacity-50 z-[70] flex items-center justify-center p-4">
                    <div className="bg-white rounded-2xl shadow-2xl p-8 max-w-sm w-full text-center">
                        ...
                        <p className="text-gray-600">Your payment was successful and your course has been booked.</p>
                        <p className="text-xs text-gray-400 mt-3">Redirecting to training page...</p>
                    </div>
                </div>
            )}
        </div>
