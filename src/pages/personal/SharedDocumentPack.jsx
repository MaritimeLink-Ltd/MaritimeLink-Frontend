...
import { FileText, Eye, AlertCircle, Loader2, Download } from 'lucide-react';
 ...
 */
function sharedFileUrl(token, documentId, { download = false } = {}) {
    const base = API_CONFIG.BASE_URL.replace(/\/+$/, '');
    return `${base}${sharedFilePath(token, documentId)}${download ? '?download=1' : ''}`;
}
    ...
    const handleDownload = async () => {
        ...
        try {
            await downloadDocumentAsPdf(
                sharedFileUrl(token, selectedId, { download: true }),
                selectedDoc?.name || 'document',
            );
        } catch (e) {
            setPreviewError(e?.message || 'Failed to download document.');
        } finally {
                    ...
                    <>
                        <div className="rounded-2xl border border-slate-200 bg-white p-4 mb-6 shadow-sm">
                            <div className="flex flex-wrap items-center gap-3 text-sm">
                                {pack.allowDownload ? (
                                    <span className="inline-flex items-center gap-1.5 rounded-full bg-green-50 text-green-700 px-3 py-1 font-medium">
                                        <Download className="w-4 h-4" />
                                        Downloading is enabled for this link
                                    </span>
                                ) : (
                                    ...
                                    {selectedDoc && selectedId ? (
                                        <>
                                            <div className="px-4 py-2 border-b border-slate-100 flex items-center justify-between gap-3">
                                                <span className="text-xs text-slate-500 truncate">{selectedDoc.name}</span>
                                                {pack.allowDownload && (
                                                    <button
                                                        type="button"
                                                        onClick={handleDownload}
                                                        disabled={isDownloading}
                                                        className="inline-flex items-center gap-1.5 rounded-lg bg-[#003366] px-3 py-1.5 text-xs font-semibold text-white hover:bg-[#002855] transition-colors flex-shrink-0 disabled:opacity-60"
                                                    >
                                                        {isDownloading ? (
