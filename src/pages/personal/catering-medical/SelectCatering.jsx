...
const SelectCatering = () => {
  ...
  const [selectedCatering, setSelectedCatering] = useState('Chief Cook');
  ...
  const handleNext = async () => {
    const subcategory = otherRole.trim() || selectedCatering;
        ...
        <div className="max-w-md w-full mx-auto lg:mx-0">
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              {cateringRoles.map((role) => (
                <button
                  key={role}
                  onClick={() => setSelectedCatering(role)}
                  className={`px-6 py-2.5 rounded-full font-medium transition-all ${selectedCatering === role
