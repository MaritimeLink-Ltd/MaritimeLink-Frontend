import { useState } from 'react';
import { useNavigate } from 'react-router-dom';
import authService from '../../../services/authService';

const SelectMedical = () => {
  const navigate = useNavigate();
  const [selectedMedical, setSelectedMedical] = useState('Ship Doctor');
  const [otherRole, setOtherRole] = useState('');
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState('');

  const medicalRoles = [
    'Ship Doctor',
    'Ship Nurse',
    'Paramedic'
  ];

  const handleNext = async () => {
    const subcategory = otherRole.trim() || selectedMedical;
    if (!subcategory) return;

    const professionalId = localStorage.getItem('professionalId');
    if (!professionalId) {
      setError('Session expired. Please start registration again.');
      return;
    }

    setLoading(true);
    setError('');
    try {
      await authService.selectRole({ professionalId, subcategory });
      navigate('/catering-medical-dashboard', { replace: true });
    } catch (err) {
      setError(err.data?.message || err.message || 'Failed to set role. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  const handleGoBack = () => {
    navigate(-1);
  };


  return (
    <div className="h-screen flex overflow-hidden">
      {/* Left Section */}
      <div className="w-full lg:w-2/5 flex flex-col justify-center px-8 sm:px-12 lg:px-16 xl:px-24 bg-white overflow-y-auto">
        <div className="max-w-md w-full mx-auto lg:mx-0">
          {/* Logo */}
          <div className="mb-4 -ml-2">
            <img
              src="/images/logo.png"
              alt="MaritimeLink Logo"
              className="w-28 h-auto"
            />
          </div>

          {/* Content */}
          <div>
            <h1 className="text-3xl font-bold text-gray-900 mb-2">
              Select Medical
            </h1>
            <p className="text-gray-500 mb-6">Choose Your Medical Role</p>

            {/* Medical Options - Pill Style */}
            <div className="flex flex-wrap gap-3 mb-6">
              {medicalRoles.map((role) => (
                <button
                  key={role}
                  onClick={() => setSelectedMedical(role)}
                  className={`px-6 py-2.5 rounded-full font-medium transition-all ${selectedMedical === role
                    ? 'bg-[#003971] text-white'
                    : 'bg-white text-gray-700 border border-gray-300 hover:border-gray-400'
                    }`}
                >
                  {role}
                </button>
              ))}
            </div>

            {/* Other Option */}
            <div className="mb-8">
              <label className="block text-gray-700 font-medium mb-2">
                Other
              </label>
              <input
                type="text"
                placeholder="Write your role"
                value={otherRole}
                onChange={(e) => setOtherRole(e.target.value)}
                className="w-full px-4 py-3 border border-gray-300 rounded-md focus:outline-none focus:ring-1 focus:ring-[#003971] focus:border-[#003971]"
              />
            </div>

            {/* Buttons */}
            {error && (
              <p className="text-sm text-red-600 mb-3">{error}</p>
            )}
            <button
              onClick={handleNext}
              disabled={loading}
              className="w-full bg-[#003971] text-white py-3 px-4 rounded-md hover:bg-[#002855] transition-colors duration-200 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? 'Saving...' : 'Next'}
            </button>

            <button
              onClick={handleGoBack}
              className="w-full text-gray-400 py-3 rounded-md font-medium hover:text-gray-600 transition-colors mt-3"
            >
              Go Back
            </button>
          </div>
        </div>
      </div>

      {/* Right Section - Image */}
      <div className="hidden lg:flex lg:w-3/5 relative py-8 lg:py-12 xl:py-16 pl-4 lg:pl-6 xl:pl-8 pr-8 lg:pr-12 xl:pr-16 items-start justify-center bg-gray-50">
        <img
          src="/images/medical-catering.webp"
          alt="Maritime Medical"
          className="w-[735px] max-h-full object-cover rounded-[15px]"
        />
      </div>
    </div>
  );
};

export default SelectMedical;
