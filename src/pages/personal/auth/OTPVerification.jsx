...
function OTPVerification() {
  ...
  // Get email and professionalId from navigation state
  const email = location.state?.email || '';
  const storedProfessionalId = location.state?.professionalId || authService.getProfessionalId();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate to select profession page (Changed flow)
    ...
    <div className="h-screen flex overflow-hidden">
      ...
      <div className="w-full lg:w-2/5 flex flex-col justify-center px-6 sm:px-12 lg:px-16 xl:px-24 bg-white overflow-y-auto">
        <div className="max-w-md w-full mx-auto lg:mx-0">
          ...
          <p className="text-sm text-gray-400 mb-2">{email || 'your email'}</p>

          {/* Timer */}
          <p className="text-sm font-medium text-gray-900 mb-6">{formatTime(timer)}</p>
