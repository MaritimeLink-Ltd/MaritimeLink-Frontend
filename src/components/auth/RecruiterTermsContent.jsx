import {
    RECRUITER_TERMS_META,
    RECRUITER_TERMS_SECTIONS,
    RECRUITER_TERMS_TITLE,
} from '../../content/recruiterTermsCopy';
...
function RecruiterTermsContent({ className = '' }) {
    return (
        <div className={`space-y-6 ${className}`.trim()}>
            <div className="space-y-1 text-sm text-gray-600">
                {RECRUITER_TERMS_META.map((line) => (
                    <p key={line}>{linkifyText(line)}</p>
                ))}
            </div>

            <div className="space-y-8">
                {RECRUITER_TERMS_SECTIONS.map((section) => (
...
export { RECRUITER_TERMS_TITLE };
export default RecruiterTermsContent;
