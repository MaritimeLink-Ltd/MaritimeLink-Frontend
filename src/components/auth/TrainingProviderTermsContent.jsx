import {
    TRAINING_PROVIDER_TERMS_META,
    TRAINING_PROVIDER_TERMS_SECTIONS,
    TRAINING_PROVIDER_TERMS_TITLE,
} from '../../content/trainingProviderTermsCopy';
...
function TrainingProviderTermsContent({ className = '' }) {
    return (
        <div className={`space-y-6 ${className}`.trim()}>
            <div className="space-y-1 text-sm text-gray-600">
                {TRAINING_PROVIDER_TERMS_META.map((line) => (
                    <p key={line}>{linkifyText(line)}</p>
                ))}
            </div>

            <div className="space-y-8">
                {TRAINING_PROVIDER_TERMS_SECTIONS.map((section) => (
...
export { TRAINING_PROVIDER_TERMS_TITLE };
export default TrainingProviderTermsContent;
